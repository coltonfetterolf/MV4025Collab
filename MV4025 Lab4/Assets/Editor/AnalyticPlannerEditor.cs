...
public class AnalyticPlannerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        planner.defensiveArc = EditorGUILayout.FloatField("defensiveArc", planner.defensiveArc);
        planner.defensiveObsvWeight = EditorGUILayout.FloatField("defensiveObsvWeight", planner.defensiveObsvWeight);
        planner.defensiveHitWeight = EditorGUILayout.FloatField("defensiveHitWeight", planner.defensiveHitWeight);
        planner.defensiveCommWeight = EditorGUILayout.FloatField("defensiveCommWeight", planner.defensiveCommWeight);
        ...
        EditorGUILayout.BeginHorizontal();
        if (GUILayout.Button("Comp Def Fires"))
        {
            // Initialize
            AstarPath.active.data.Awake();
            NavGraph graph = AstarPath.active.graphs[0];
        ...
        if (GUILayout.Button("Optimize Defensive Position"))
        {
            ...
            Vector3 rawDefCM = CMOfTag("RedForce");
            Vector3 defCM = rawDefCM;
            defCM.y = 0f;
            float defensiveSector = planner.defensiveArc;
